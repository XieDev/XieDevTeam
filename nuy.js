...
async function starts() {
    ...
    nuy.on('group-participants-update', async (anu) => {
       ...
       if (anu.action == 'add') {
            ...
            teks = `*Welcome* @${num.split('@')[0]}\n\nMohon Baca Terlebih Dahulu Deskripsi!`
            ...
            imgnya = await nuy.prepareMessage(buffer, location, {thumbnail: buffer})
            buttons = [
            ...
            const buttonMessages = {
               ...
               contentText: `${teks}`,
               footerText: "Note\nJika Anda Ingin Menggunakan Bot Xie Silahkan Klik Dibawah\n\n𝐴𝑢𝑡ℎ𝑜𝑟 𝐹𝑥𝑆𝑥",
               buttons: buttons,
               headerType: 6
            }
            nuy.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Welcome @${num.split('@')[0]}`, orderTitle: `Welcome @${num.split('@')[0]}`, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, caption: teks})
            }
            if (anu.action == 'remove') {
                ...
                anu_user = nuy.contacts[mem]
                member = mdata.participants.length
                jametout = await fs.readFileSync('./mp3/jamet.mp3')
                teks = `*Sayonara* @${num.split('@')[0]}\n\nSi Jamet Kenapa Keluar Dari Grup`
                ...
                nuy.sendMessage(mdata.id, jametout, MessageType.audio, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Keluar @${num.split('@')[0]}`, orderTitle: `Keluar @${num.split('@')[0]}`, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, caption: teks})
            }
		} catch (e) {
	...
	nuy.on('chat-update', async (caa) => {
		try {
         ...
         const sendProduct = (teks) => {
        	    duct = nuy.prepareMessageFromContent(from, {"productMessage": {"product": {"productImage": {"jpegThumbnail": fs.readFileSync('pinky.jpg')},"title": "Xie Bot","description": teks,"currencyCode": "IDR","priceAmount1000": "0","productImageCount": 1,"salePriceAmount1000": "0"},"businessOwnerJid": "6283818221226@s.whatsapp.net"}}, {quoted: ftrolii})
             nuy.relayWAMessage(duct)
         }
         const replycaa = (teks) => {
             buttons = [{buttonId: `${prefix}daftar`, buttonText: {displayText: 'Daftar User'}, type: 1}]
             buttonsMessage = {contentText: teks, footerText: "𝐴𝑢𝑡ℎ𝑜𝑟 𝐹𝑥𝑆𝑥", buttons: buttons, headerType: 1}
             prep = await nuy.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrolii})
         ...
         if (!caa.key.fromMe) return
            ...
            if (budy.toLowerCase().startsWith(`${prefix}unmute`)){
                ...
                sendProduct(`Xie Bot Telah Diunmute, Di Grup Ini`)
            }
         }
      ...
      if (budy.startsWith('>')){
        ...
        replyca(e)
            ...
            case 'help':
               ...
               if (!isUser) return replycaa(mess.only.userXie)
...
╰──────𖧹`
            ...
            case 'mute':
               ...
               mutebot.push(from)
               fs.writeFileSync('./database/mute.json', JSON.stringify(mutebot))
               sendProduct(`Xie Bot Telah Dimute, Di Grup Ini`)
               break
            case 'return':
				case '>':
				   if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
...
// last
            ...
            case 'style':
               ...
               daftarimg = await getBuffer(ppimg)
             	imgnya = await nuy.prepareMessage(from, daftarimg, location, {thumbnail: daftarimg})
               gbutsan = [
                 {buttonId: `${prefix}daftarmenu`, buttonText: {displayText: 'Daftar Menu'}, type: 1},
                 {buttonId: `${prefix}rules`, buttonText: {displayText: 'Rules Bot'}, type: 1}
               ]
               buttonMessages = {
                 ...
                 footerText: `Jika Anda Ingin Menggunakan Bot Xie Silahkan Klik Dibawah\n\n𝐴𝑢𝑡ℎ𝑜𝑟 𝐹𝑥𝑆𝑥`,
                 buttons: gbutsan,
                 headerType: 4
               }
               await nuy.sendMessage(from, buttonMessages, MessageType.buttonsMessage, {quoted: ftrolii, caption: captionnya, contextInfo: {mentionedJid: [sender]}})
             	break
            case 'profile':
    				nuy.updatePresence(from, Presence.composing)
					if (!isUser) return replyca(mess.only.userXie)
